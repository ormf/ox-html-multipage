...
still inferior to file-local settings."
  (let ((parsed-keywords
    ...
    ;; In case of multipage output compute the subtrees to export plus
    ;; additional info like filenames for further processing and store
    ;; them as properties in info.
    (when (plist-get info :multipage-process-hook)
      (setq info
            (funcall
             (plist-get info :multipage-process-hook)
             info)))
